...
public class TambahBeratL extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_tambah_berat_l);
        setTitle("Tambah Batas Berat Laki-laki");
    ...
    public boolean validate() {
        ...
        if (b1.isEmpty()) {
            input_b1.setError("Berat ke-1 berapa?");
            valid = false;
        } else {
        ...
        if (b2.isEmpty()) {
            input_b2.setError("Berat ke-2 berapa?");
            valid = false;
        } else {
        ...
        if (b3.isEmpty()) {
            input_b3.setError("Berat ke-3 berapa?");
            valid = false;
        } else {
        ...
        if (b4.isEmpty()) {
            input_b4.setError("Berat ke-4 berapa?");
            valid = false;
        } else {
        ...
        if (b5.isEmpty()) {
            input_b5.setError("Berat ke-5 berapa?");
            valid = false;
        } else {
        ...
        if (b6.isEmpty()) {
            input_b6.setError("Berat ke-6 berapa?");
            valid = false;
        } else {
        ...
        if (b7.isEmpty()) {
            input_b7.setError("Berat ke-7 berapa?");
            valid = false;
        } else {
    ...
    class AttemptSubmit extends AsyncTask<String, String, String> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            pDialog = new ProgressDialog(TambahBeratL.this);
        ...
        protected String doInBackground(String... args) {
            ...
            nameValuePairs.add(new BasicNameValuePair("berat1", b1));
            nameValuePairs.add(new BasicNameValuePair("berat2", b2));
            nameValuePairs.add(new BasicNameValuePair("berat3", b3));
            nameValuePairs.add(new BasicNameValuePair("berat4", b4));
            nameValuePairs.add(new BasicNameValuePair("berat5", b5));
            nameValuePairs.add(new BasicNameValuePair("berat6", b6));
            nameValuePairs.add(new BasicNameValuePair("berat7", b7));
            try {
                HttpClient httpClient = new DefaultHttpClient();
                HttpPost httpPost = new HttpPost(
                        "http://posyanduanak.com/kenanga/insert_berat_l.php");
...
//            Toast.makeText(getApplicationContext(), result, Toast.LENGTH_LONG).show();
            pDialog.dismiss();
            if (result != null) {
                Toast.makeText(TambahBeratL.this, result, Toast.LENGTH_LONG).show();
            }
            input_b1.setText("");
            input_b2.setText("");
            input_b3.setText("");
