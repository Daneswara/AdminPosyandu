...
import com.masbie.adminposyandu.DetailRiwayat;
...
import com.masbie.adminposyandu.pengaturan.PengaturanGrafikTinggiP;

public class AdapterGrafik extends BaseAdapter {
    private Context mContext;

    // Constructor
    public AdapterGrafik(Context c) {
        mContext = c;
    }
        ...
        if (convertView == null) {
            ...
            lin.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    Intent intent;
                    if (position == 0) {
                        intent = new Intent(mContext, PengaturanGrafikTinggiL.class);
                        mContext.startActivity(intent);
                    } else if (position == 1) {
                        intent = new Intent(mContext, PengaturanGrafikTinggiP.class);
                        mContext.startActivity(intent);
                    } else if (position == 2) {
                        intent = new Intent(mContext, PengaturanGrafikBeratL.class);
                        mContext.startActivity(intent);
                    } else if (position == 3) {
                        intent = new Intent(mContext, PengaturanGrafikBeratP.class);
                        mContext.startActivity(intent);
                    }
                }
            });
    ...
    public String[] pelajaran = {
            "Grafik Tinggi (Laki-laki)", "Grafik Tinggi (Perempuan)",
            "Grafik Berat (Laki-laki)", "Grafik Berat (Perempuan)"
    };
}